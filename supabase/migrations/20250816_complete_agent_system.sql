...
CREATE TABLE IF NOT EXISTS agent_sessions (
  ...
  created_at TIMESTAMPTZ DEFAULT NOW(),
  INDEX idx_agent_sessions_agent_fid (agent_fid),
...
CREATE TABLE IF NOT EXISTS funding_intents (
  ...
  created_at TIMESTAMPTZ DEFAULT NOW(),
  INDEX idx_funding_intents_loan (loan_id),
  INDEX idx_funding_intents_lender (lender_fid),
  INDEX idx_funding_intents_created (created_at)
);
...
-- SELECT COUNT(*) FROM human_autolend_prefs;

-- Migration complete! Agent lending system is now enabled.